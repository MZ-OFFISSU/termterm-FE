 ...
 * 단어 카드
 */
const WordCard = ({ word, ...props }: Props) => {
  ...
  return (
    <Container COLOR={COLOR} mode={mode} {...props}>
      <NameWrapper>
        {sub ? (
          <SubName COLOR={COLOR} mode={mode}>
            {sub}
          </SubName>
        ) : (
          <></>
        )}
        <MainName COLOR={COLOR} mode={mode} style={{ marginTop: 5 }}>
          {main}
        </MainName>
      </NameWrapper>
...
const SubName = styled.Text<{ COLOR: colorTheme; mode: boolean }>`
  font-size: ${TEXT_STYLES["2xsm"].Md?.fontSize}px;
  font-weight: ${TEXT_STYLES["2xsm"].Md?.fontWeight};
  color: ${(props) =>
    props.mode ? props.COLOR.THEME.primary[160] : "#929292"};
  text-align: center;
`;

const MainName = styled.Text<{ COLOR: colorTheme; mode: boolean }>`
...
const Source = styled.Text<{ COLOR: colorTheme; mode: boolean }>`
  font-size: ${TEXT_STYLES["3xsm"].Md?.fontSize}px;
  font-weight: ${TEXT_STYLES["3xsm"].Md?.fontWeight};
  color: ${(props) =>
    props.mode ? props.COLOR.THEME.primary.variant : props.COLOR.Neutral[10]};
`;
